    ...
    match ast {
        AstNode::Assignment(vtype, name, exp) => {
            let variable_type: VarType = match vtype {
                Some(var_type) => match var_type {
                    VarType::String => {
                        state
                            .var_map
                            .insert(name.clone(), Constant::String("".to_string()));
                        VarType::String
                    }
                    VarType::Int => {
            ...
            match (variable_type, eval_expr(exp, state)?) {
                ...
                (VarType::Float, Constant::Float(f)) => {
                    state.var_map.insert(name, Constant::Float(f))
                }
                (VarType::String, Constant::String(s)) => {
                    state.var_map.insert(name, Constant::String(s))
                }
                (_, _) => {
        ...
        AstNode::ArrayDef(var_type, name, piped, value_exp, length_exp) => {
            ...
            state
                .var_map
                .insert(name, Constant::Array(var_type, elements));
        }
...
fn eval_bool(bool_exp: &BoolExp, state: &mut State) -> Result<bool, ExecutionError> {
    ...
    use Constant::*;
    let (lres, rres) = match (
        eval_expr(lhs.clone(), state)?,
        eval_expr(rhs.clone(), state)?,
    ) {
                ...
                Object::FuncCall(func_call) => {
                    ...
                    for (expr, (var_type, param_name)) in func_call.params.iter().zip(params.iter())
                    {
                        ...
                        match (var_type, &param_const) {
                            (VarType::Int, Constant::Int(_))
                            | (VarType::Int, Constant::Array(_, _))
                            | (VarType::Float, Constant::Float(_))
                            | (VarType::String, Constant::String(_)) => {
